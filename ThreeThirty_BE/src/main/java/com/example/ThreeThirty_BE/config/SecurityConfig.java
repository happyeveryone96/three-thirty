...
import com.example.ThreeThirty_BE.security.auth.util.CookieAuthorizationRequestRepository;
import com.example.ThreeThirty_BE.security.auth.util.OAuth2AuthenticationFailureHandler;
import com.example.ThreeThirty_BE.security.auth.util.OAuth2AuthenticationSuccessHandler;
import com.example.ThreeThirty_BE.security.jwt.exception.CustomAuthenticationEntryPoint;
import com.example.ThreeThirty_BE.service.login.CustomOAuth2UserService;
...
public class SecurityConfig {
  ...
  public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    return http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS)
        ...
        .antMatchers("/", "/users", "/users/login", "/users/logout", "/post","/users/refreshToken", "/webcam", "/practice/start", "/Learning/quiz", "/images/{imageName}").permitAll()
        ...
        .authorizationRequestRepository(
            cookieAuthorizationRequestRepository)   // redirect를 관리 할 repository
        .and()
